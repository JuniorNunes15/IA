...
public class GreedyPartialOrderLifter implements PartialOrderLifter {
	...
	public static PartialOrderPlan lift(TotalOrderPlan top, GroundProblem problem) {
		 ...
		 * //Must create new instances for where the same action is performed more than
		 * once List TOPlan = new ArrayList(); Iterator pit = pTOPlan.iterator(); while
		 * (pit.hasNext()) { InstantAction a = (InstantAction) pit.next();
		 * TOPlan.add(a.clone()); }
		 */
