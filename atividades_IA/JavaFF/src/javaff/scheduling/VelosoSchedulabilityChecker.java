...
public class VelosoSchedulabilityChecker implements SchedulabilityChecker, Cloneable {
	...
	private void add(EnvelopeEntry e, InstantAction a, TemporalMetricState s) {
		...
		/*
		 * Set ps = new HashSet(); Iterator pit = e.precedesEnd.iterator(); while
		 * (pit.hasNext()) { InstantAction p = (InstantAction) pit.next(); if
		 * (checkOrder(a,p)) ps.add(p); } pit = ps.iterator(); while (pit.hasNext()) {
		 * InstantAction p = (InstantAction) pit.next(); e.addPrecederOrder(a,p,s); }
		 */
